    ...
    private static async Task SlashCommandHandler(SocketSlashCommand command)
    {
        switch (command.Data.Name)
        {
            case "current-time":
                ...
                await command.RespondAsync($"It is {formattedTime} in @{command.User.Id} TimeZone (sent as {userTimeZone} btw)"); // Only returns the time
                return;
            case "make-sandwich":
                ...
                await command.RespondAsync($"@{command.User.Id}Here is your {responseMessage} sandwich.");
                return;
            case "current-day":
